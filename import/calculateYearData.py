...
def main():
  ...
  for year in range (2011, 2014):
    ...
    if ( countv > 0 ):
      ...
      if c.execute('SELECT * from year_data WHERE date='+date+';').fetchone() is not None:
        insert = 'UPDATE year_data SET total_power_kWh='+str(maxv)+',power_kWh='+str(sumv)+' WHERE date='+date+';'
      print insert
      conn.execute(insert)
  conn.commit()
  conn.close()
